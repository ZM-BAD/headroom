...
这是因为 **context window 有上限**，当它被填满后，AI 会悄无声息地丢弃早期信息。所有主流 AI 聊天平台都没有告诉你 context 还剩多少。
...
- **7 个平台** — DeepSeek、ChatGPT、Gemini、Kimi、Qwen、通义千问、豆包 (Doubao)。
- **实时仪表盘** — 边聊边看 context window 使用量的变化。绿/黄/红三色阈值告诉你什么时候该开新对话。
- **逐轮明细** — 每一轮问答的 token 估算量，输入和输出分别展示。
...
2. **打开侧边栏** — 在任何支持的 AI 聊天平台上，Headroom 读取对话历史并估算 token 用量
3. **看着仪表盘** — 边聊边看 context 剩余空间的变化

扩展完全离线运行。它从页面读取对话文本来计数，然后只保留计数。
