...
1. **Token 估算系数校准（正确性）**：把 001 的「平台 × 书写系统」系数矩阵从 v1（中文/英文）扩到更多书写系统，并按各平台真实 tokenizer 精确校准。
2. **跨浏览器深度 QA（可移植性）**：开发以 Chrome 为主，Edge 基本跟随，Firefox 补齐边角交互差异。

## 主题 1：Token 系数校准

**现状（001）**：CJK + Latin 两书写系统，DeepSeek 系数为起点值（`cjk 0.6 / latin 0.5`，待标定）；其余平台沿用同值。

**目标**：

- 扩书写系统：西里尔、阿拉伯、日文假名、等——给每种书写系统独立系数（v1 都归 Latin 桶，偏差大）。
- 按平台 tokenizer 校准：同一书写系统在不同平台系数不同（DeepSeek 与 Qwen/GPT 的汉字系数差异等）。
- 标定基准：可借用平台服务端返回的 token 数（DeepSeek/Qwen/通义千问有）作参照，对照估测值回归系数。

**验收**：估算值 vs 真实 tokenizer，各书写系统/平台落在给定误差带内。
